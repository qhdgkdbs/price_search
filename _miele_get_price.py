...
form_class = uic.loadUiType("miele_get_price.ui")[0]
...
class WindowClass(QMainWindow, form_class) :
    def __init__(self) :
        ...
        # 버튼에 기능을 연결하는 코드
        self.btn_start.clicked.connect(self.btn_start_function)
        self.btn_base.clicked.connect(self.btn_base_function)
        self.btn_excel.clicked.connect(self.btn_excel_function)

        # self.now_data.setText("hello")
        # self.now_data.clicked.connect(self.label_now_data_function)

    def open_base_data_file(self):
        if sys.platform == "win32":
            os.startfile("url.txt")
        else:
            os.system("open url.txt")

    def open_excel_data_file(self):
        if sys.platform == "win32":
            os.startfile("price.xlsx")
        else:
            os.system("price.xlsx")
    ...
    def btn_start_function(self):
        ...
        with codecs.open("url.txt", 'r', encoding='utf8') as f:
            lines = f.read().split('\n')
        ...
        for n, index in enumerate(lines):
            ...
            data.insert(len(data), [info_product.split(" ")[0], info_product.split(" ")[1], info_product.split(" ")[2],
                                    info_product.split(" ")[3]])
            ...
            for i in range(0, price_info_numbers):
                ...
                if (soup.find_all('a', class_='basicList_mall__sbVax')[i].img):
                    ...
                    if (mall_name != '쇼핑몰별 최저가'):
                        ...
                        shipping_price = shipping_price_box.find_all('em', class_='basicList_option__3eF2s')[
                            0].text.replace(',', '')
                    ...
                    if shipping_price:
                        ...
                        data_arr.insert(len(data_arr),
                                        [int(price) + shipping_price, "네이버[" + mall_name + "-배송비 포함]", url, title])
        ...
        print("btn Pressed")
        self.change_now_data('Save Data File')

    # btn_2가 눌리면 작동할 함수
    def btn_base_function(self):
        # self.now_data.setText("open Base Data")
        # print("btn Pressed")
        self.change_now_data('Open Data File')
        self.open_base_data_file()

    def btn_excel_function(self):
        # print("btn Pressed")
        self.change_now_data('Open Excel File')
        self.open_excel_data_file()


    def change_now_data(self, text):
        self.now_data.setText(text)

    # def label_now_data_function(self):
