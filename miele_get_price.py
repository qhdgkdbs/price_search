...
class WindowClass(QMainWindow, form_class) :
    def __init__(self) :
        ...
        self.btn_ship.clicked.connect(self.btn_ship_function)
    ...
    def btn_start_function(self):
        ...
        # 배송비가 있는 상점들
        f = open("mall_with_shipping_price.txt", "r")
        mall_with_shippping_price = f.read().split('\n')
        f.close()
        ...
        for n, index in enumerate(lines):
            ...
            # 데이터의 갯수를 제한하는 단계
            if (price_info_numbers > 8):
                price_info_numbers = 8
            else:
                pass
            ...
            for i in range(0, price_info_numbers):
                ...
                if (soup.find_all('a', class_='basicList_mall__sbVax')[i].img):
                    ...
                    if mall_name in mall_with_shippping_price:
                        # 네이버 스토어 중에서 배송비가 있는 곳
                        data_arr.insert(len(data_arr), [int(price) + 100000, "네이버[" + mall_name + "]", url, title])
                    else:
